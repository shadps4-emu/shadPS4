...
std::tuple<ImageId, int, int> TextureCache::ResolveOverlap(const ImageInfo& image_info,
    ...
    if (image_info.guest_address == cache_image.info.guest_address) {
        ...
        LOG_ERROR(
            ...
            "  Old vs expected diff:  {} bytes\n"
            "  New vs expected diff:  {} bytes\n"
            "  Merged image ID:       {}\n"
            "  Binding type:          {}\n"
            "  Current tick:          {}\n"
            ...
            vk::to_string(cache_image.info.pixel_format), static_cast<int>(cache_image.info.type),
            cache_image.info.size.width, cache_image.info.size.height, cache_image.info.size.depth,
            cache_image.info.pitch, // Added pitch
            ...
            cache_image.info.num_bits, cache_image.info.props.is_block, cache_image.info.guest_size,
            cache_image.tick_accessed_last, safe_to_delete,

            // New image details
            image_info.guest_address, image_info.guest_size, vk::to_string(image_info.pixel_format),
            static_cast<int>(image_info.type), image_info.size.width, image_info.size.height,
            image_info.size.depth,
            image_info.pitch, // Added pitch
            image_info.resources.levels, image_info.resources.layers, image_info.num_samples,
            static_cast<u32>(image_info.tile_mode), image_info.num_bits, image_info.props.is_block,
            ...
            (image_info.pitch == cache_image.info.pitch), // Added pitch comparison
            (cache_image.info.resources <= image_info.resources), cache_image.info.resources.levels,
            image_info.resources.levels, (cache_image.info.guest_size <= image_info.guest_size),
            ...
            // Difference between actual and expected sizes
            static_cast<s64>(cache_image.info.guest_size) -
                static_cast<s64>(static_cast<u64>(image_info.size.width) *
                                 static_cast<u64>(image_info.size.height) *
                                 static_cast<u64>(image_info.size.depth) *
                                 static_cast<u64>(image_info.num_bits) / 8),

            static_cast<s64>(image_info.guest_size) -
                static_cast<s64>(static_cast<u64>(image_info.size.width) *
                                 static_cast<u64>(image_info.size.height) *
                                 static_cast<u64>(image_info.size.depth) *
                                 static_cast<u64>(image_info.num_bits) / 8),
