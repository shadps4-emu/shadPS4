...
public:
    ...
    [[nodiscard]] std::optional<std::vector<u32>> Load(std::string_view shader_name,
                                                       std::string_view generation,
                                                       std::string_view permutation);
    [[nodiscard]] bool Store(std::string_view shader_name, std::string_view generation,
                             std::string_view permutation, std::span<const u32> spirv);

private:
    [[nodiscard]] std::optional<std::filesystem::path> PrepareGenerationLocked(
