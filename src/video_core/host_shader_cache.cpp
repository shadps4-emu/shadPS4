...
static_assert(std::is_trivially_copyable_v<CacheHeader> && sizeof(CacheHeader) == 36);

bool IsSafeName(std::string_view value, bool allow_period) {
    if (value.empty() || value == "." || value == ".." ||
        (allow_period && (value.front() == '.' || value.back() == '.' ||
                          value.find("..") != std::string_view::npos))) {
        return false;
    }
    return std::ranges::all_of(value, [allow_period](char c) {
...
bool Publish(const std::filesystem::path& source, const std::filesystem::path& destination) {
    ...
    if (ec) {
        ...
        return std::filesystem::is_regular_file(destination, ec);
    }
    return true;
}
...
std::optional<std::filesystem::path> HostShaderCache::PrepareGenerationLocked(
    ...
    if (auto it = prepared_generations.find(std::string{shader_name});
        it != prepared_generations.end() && it->second == generation) {
    ...
    if (ec || (std::filesystem::create_directories(generation_path, ec), ec))
        return std::nullopt;
...
std::optional<std::vector<u32>> HostShaderCache::Load(std::string_view shader_name,
    ...
    if (!IsSafeName(permutation, false))
        return std::nullopt;
    const auto gen_path = PrepareGenerationLocked(shader_name, generation);
    if (!gen_path)
        return std::nullopt;

    std::ifstream file{*gen_path / (std::string{permutation} + ".spv"),
    ...
    file.read(reinterpret_cast<char*>(spirv.data()), spirv.size() * sizeof(u32));
...
bool HostShaderCache::Store(std::string_view shader_name, std::string_view generation,
    ...
    const auto destination = *gen_path / (std::string{permutation} + ".spv");
    const CacheHeader header{CacheMagic, CacheVersion, static_cast<u32>(spirv.size()),
                             Digest(spirv)};
    ...
    file.write(reinterpret_cast<const char*>(spirv.data()), spirv.size_bytes());
    file.flush();
    file.close();
    std::error_code ec;
    if (!file) {
