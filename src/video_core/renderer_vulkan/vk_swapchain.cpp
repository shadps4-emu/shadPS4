...
void Swapchain::RefreshSemaphores() {
    ...
    const bool has_maint1 = instance.HasSwapchainMaintenance1();
    for (u32 i = 0; i < image_count; ++i) {
        auto [acq_res, acq_sem] = device.createSemaphore({});
        ASSERT_MSG(acq_res == vk::Result::eSuccess, "Failed to create image acquired semaphore: {}",
                   vk::to_string(acq_res));
        image_acquired[i] = acq_sem;
        SetObjectName(device, acq_sem, "Swapchain Semaphore: image_acquired {}", i);

        auto [rdy_res, rdy_sem] = device.createSemaphore({});
        ASSERT_MSG(rdy_res == vk::Result::eSuccess, "Failed to create present ready semaphore: {}",
                   vk::to_string(rdy_res));
        present_ready[i] = rdy_sem;
        SetObjectName(device, rdy_sem, "Swapchain Semaphore: present_ready {}", i);

        if (has_maint1) {
            auto [fnc_res, fnc] = device.createFence({.flags = vk::FenceCreateFlagBits::eSignaled});
            ASSERT_MSG(fnc_res == vk::Result::eSuccess,
                       "Failed to create swapchain present fence: {}", vk::to_string(fnc_res));
            present_fences[i] = fnc;
            SetObjectName(device, fnc, "Swapchain Fence: present_done {}", i);
        }
    }
}
