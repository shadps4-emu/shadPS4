...
bool CompactArchive(const std::filesystem::path& source, const std::filesystem::path& destination) {
    ...
    std::unordered_map<std::string_view, mz_uint> last_entries;
    ...
    bool succeeded = true;
    for (mz_uint index = 0; index < num_files; ++index) {
        if (last_entries.find(names[index])->second == index &&
            !mz_zip_writer_add_from_zip_reader(&writer, &reader, index)) {
            succeeded = false;
            break;
        }
    }
    succeeded = succeeded && mz_zip_writer_finalize_archive(&writer);
