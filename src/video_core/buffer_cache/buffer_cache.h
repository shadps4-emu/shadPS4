...
private:
    ...
    ArenaBinds* BindsForArena(const Arena* arena) {
    ...
    Arena* CreateArena(u64 base_block, u64 num_pages);

    const Arena* GetArena(u64 first_block, u64 last_block);

    void EnsureResident(const Arena* arena, u64 first_block, u64 last_block);

    void DownloadMemory(const Arena* arena, VAddr device_addr, u64 size);

    bool SynchronizeMemory(const Arena* arena, VAddr device_addr, u32 size, bool is_written,
    ...
    std::array<const Arena*, NUM_ARENA_PAGES> address_space{};
