...
public:
    ...
    void ChangeRegionState(u64 offset, u64 size) {
        ...
        if (update_watchers &&
            (EmulatorSettings.GetReadbacksMode() == GpuReadbacksMode::Precise || !is_gpu)) {
    ...
    template <Type type, LockOp lock_op>
        requires(std::popcount(std::bit_cast<std::underlying_type_t<LockOp>>(lock_op)) <= 1)
    bool IsRegionModified(u64 offset, u64 size) noexcept {
        auto& state = GetRegionBits<type>();
        const auto do_lock_op = [this](u64 word) {
            if constexpr (lock_op == LockOp::Lock) {
                locks[word].lock();
            } else if constexpr (lock_op == LockOp::Unlock) {
                locks[word].unlock();
            }
        };
        const auto [start_word, start_page, end_word, end_page] = GetBounds(offset, size);
        const auto [start_mask, end_mask] = GetMasks(start_page, end_page);
        do_lock_op(start_word);
        if (start_word == end_word) [[likely]] {
    ...
    void UnlockWords(u64 offset, u64 size) noexcept {
...
private:
    ...
    using LockT = Common::AdaptiveMutex;
#else
    using LockT = std::mutex;
#endif
    std::array<LockT, NUM_REGION_WORDS> locks;
