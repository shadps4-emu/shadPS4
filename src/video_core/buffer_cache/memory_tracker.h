...
public:
    ...
    bool IsRegionGpuModified(VAddr cpu_addr, u64 size) noexcept {
        return IteratePages(cpu_addr, size, [](RegionManager* manager, u64 offset, u64 size) {
            return manager->template IsRegionModified<Type::GPU, LockOp::None>(offset, size);
        });
    }
    ...
    bool IsRegionCpuModified(VAddr cpu_addr, u64 size) noexcept {
        return IteratePages(cpu_addr, size, [](RegionManager* manager, u64 offset, u64 size) {
            return manager->template IsRegionModified<Type::CPU, LockOp::None>(offset, size);
        });
    }
    ...
    void InvalidateRegion(VAddr cpu_addr, u64 size, auto&& on_flush) noexcept {
        ...
        IteratePages(cpu_addr, size, [&should_flush](RegionManager* manager, u64 offset, u64 size) {
            if (manager->template IsRegionModified<Type::GPU, LockOp::Lock>(offset, size)) {
                manager->UnlockWords(offset, size);
                should_flush = true;
                return;
            }
            manager->template ChangeRegionState<Type::CPU, LockOp::Unlock, true>(offset, size);
