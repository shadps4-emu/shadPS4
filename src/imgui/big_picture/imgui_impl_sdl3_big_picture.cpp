...
struct ImGui_ImplSDL3_Data {
    ...
    bool MouseCanReportHoveredViewport; // This is hard to use/unreliable on SDL so we'll set
                                        // ImGuiBackendFlags_HasMouseHoveredViewport dynamically
                                        // based on state.
...
static ImGui_ImplSDL3_Data* ImGui_ImplSDL3_GetBackendData() {
    return ImGui::GetCurrentContext() ? (ImGui_ImplSDL3_Data*)ImGui::GetIO().BackendPlatformUserData
                                      : nullptr;
}
...
static void ImGui_ImplSDL3_PlatformSetImeData(ImGuiContext*, ImGuiViewport* viewport,
...
bool ImGui_ImplSDL3_ProcessEvent(const SDL_Event* event) {
    ...
    case SDL_EVENT_KEY_UP: {
        ...
        //     SDL_GetKeyName(event->key.key), event->key.scancode,
        //     SDL_GetScancodeName(event->key.scancode), event->key.mod);
        ImGuiKey key = ImGui_ImplSDL3_KeyEventToImGuiKey(event->key.key, event->key.scancode);
        io.AddKeyEvent(key, (event->type == SDL_EVENT_KEY_DOWN));
        io.SetKeyEventNativeData(
            key, event->key.key, event->key.scancode,
            event->key.scancode); // To support legacy indexing (<1.87 user code). Legacy
    ...
    case SDL_EVENT_WINDOW_MOUSE_ENTER: {
        ...
        //   causing SDL_WINDOWEVENT_LEAVE on previous frame to interrupt drag operation by clear
        //   mouse position. This is why we delay process the SDL_WINDOWEVENT_LEAVE events by one
...
static bool ImGui_ImplSDL3_Init(SDL_Window* window, SDL_Renderer* renderer, void* sdl_gl_context) {
    ...
    // https://github.com/ocornut/imgui/issues/4960) We will use 'MouseCanReportHoveredViewport' to
    // set 'ImGuiBackendFlags_HasMouseHoveredViewport' dynamically each frame.
#ifndef __APPLE__
    ...
    // Check and store if we are on a SDL backend that supports SDL_GetGlobalMouseState() and
    // SDL_CaptureMouse()
    ...
    bd->MouseCursors[ImGuiMouseCursor_Progress] =
    ...
    // From 2.0.5: Set SDL hint to receive mouse click events on window focus, otherwise SDL
    // doesn'temit the event. Without this, when clicking to gain focus, our widgets wouldn't
    // activate even though they showed as hovered. (This is unfortunately a global SDL setting, so
    // enabling it might have a side-effect on your application. It is unlikely to make a
    // difference, but if your app absolutely needs to ignore the initial on-focus click: you can
    // ignore SDL_EVENT_MOUSE_BUTTON_DOWN events coming right after a SDL_WINDOWEVENT_FOCUS_GAINED)
    SDL_SetHint(SDL_HINT_MOUSE_FOCUS_CLICKTHROUGH, "1");

    // From 2.0.22: Disable auto-capture, this is preventing drag and drop across multiple
    // windows(see #5710)
    ...
    // We need SDL_CaptureMouse(), SDL_GetGlobalMouseState() from SDL 2.0.4+ to support
    // multiple viewports. We left the call to ImGui_ImplSDL3_InitPlatformInterface() outside of
    // #ifdef to avoid unused-function warnings.
...
// Should technically be a SDL_GLContext but due to typedef it is sane to keep it void* in public
// interface.
...
// This code is incredibly messy because some of the functions we need for full viewport supportare
// not available in SDL < 2.0.4.
static void ImGui_ImplSDL3_UpdateMouseData() {
    ImGui_ImplSDL3_Data* bd = ImGui_ImplSDL3_GetBackendData();
    ImGuiIO& io = ImGui::GetIO();

    // We forward mouse input when hovered or captured (via SDL_EVENT_MOUSE_MOTION) or whenfocused
    // (below)
#if SDL_HAS_CAPTURE_AND_GLOBAL_MOUSE
    // - SDL_CaptureMouse() let the OS know e.g. that our drags can extend outside of
    // parent boundaries (we want updated position) and shouldn't trigger other operations outside.
    // - Debuggers under Linux tends to leave captured mouse on break, which may be very
    // inconvenient, so to mitigate the issue we wait until mouse has moved to begin capture.
    ...
    if (is_app_focused) {
        ...
        // when io.ConfigNavMoveSetMousePos is enabled by user)
        ...
        if (bd->MouseCanUseGlobalState && bd->MouseButtonsDown == 0 && !is_relative_mouse_mode) {
            // Single-viewport mode: mouse position in client window coordinates (io.MousePos
            // is (0,0) when the mouse is on the upper-left corner of the app window) Multi-viewport
            // mode: mouse position in OS absolute coordinates (io.MousePos is (0,0) when the mouse
    ...
    // viewport the OS mouse cursor is hovering. If ImGuiBackendFlags_HasMouseHoveredViewport is not
    // set by the backend, Dear imGui will ignore this field and infer the information using its
    // flawed heuristic.
    ...
    //       viewport has the _NoInputs flag in order to allow us to find the viewport under), then
    //       Dear ImGui is forced to ignore the value reported by the backend, and use its flawed
    //       heuristic to guess the viewport behind.
    // - [X] SDL backend correctly reports this regardless of another viewport behind focused and
    // dragged from (we need this to find a useful drag and drop target).
    if (io.BackendFlags & ImGuiBackendFlags_HasMouseHoveredViewport) {
        ImGuiID mouse_viewport_id = 0;
        if (ImGuiViewport* mouse_viewport =
                ImGui_ImplSDL3_GetViewportForWindowID(bd->MouseWindowID))
...
static void ImGui_ImplSDL3_UpdateGamepadButton(ImGui_ImplSDL3_Data* bd, ImGuiIO& io, ImGuiKey key,
...
static void ImGui_ImplSDL3_UpdateGamepads() {
    ...
    ImGui_ImplSDL3_UpdateGamepadButton(bd, io, ImGuiKey_GamepadDpadDown,
                                       SDL_GAMEPAD_BUTTON_DPAD_DOWN);
    ImGui_ImplSDL3_UpdateGamepadButton(bd, io, ImGuiKey_GamepadL1,
                                       SDL_GAMEPAD_BUTTON_LEFT_SHOULDER);
    ImGui_ImplSDL3_UpdateGamepadButton(bd, io, ImGuiKey_GamepadR1,
                                       SDL_GAMEPAD_BUTTON_RIGHT_SHOULDER);
    ImGui_ImplSDL3_UpdateGamepadAnalog(bd, io, ImGuiKey_GamepadL2, SDL_GAMEPAD_AXIS_LEFT_TRIGGER,
                                       0.0f, 32767);
    ImGui_ImplSDL3_UpdateGamepadAnalog(bd, io, ImGuiKey_GamepadR2, SDL_GAMEPAD_AXIS_RIGHT_TRIGGER,
                                       0.0f, 32767);
    ImGui_ImplSDL3_UpdateGamepadButton(bd, io, ImGuiKey_GamepadL3, SDL_GAMEPAD_BUTTON_LEFT_STICK);
    ImGui_ImplSDL3_UpdateGamepadButton(bd, io, ImGuiKey_GamepadR3, SDL_GAMEPAD_BUTTON_RIGHT_STICK);
    ImGui_ImplSDL3_UpdateGamepadAnalog(bd, io, ImGuiKey_GamepadLStickLeft, SDL_GAMEPAD_AXIS_LEFTX,
                                       -thumb_dead_zone, -32768);
    ImGui_ImplSDL3_UpdateGamepadAnalog(bd, io, ImGuiKey_GamepadLStickRight, SDL_GAMEPAD_AXIS_LEFTX,
                                       +thumb_dead_zone, +32767);
    ImGui_ImplSDL3_UpdateGamepadAnalog(bd, io, ImGuiKey_GamepadLStickUp, SDL_GAMEPAD_AXIS_LEFTY,
                                       -thumb_dead_zone, -32768);
    ImGui_ImplSDL3_UpdateGamepadAnalog(bd, io, ImGuiKey_GamepadLStickDown, SDL_GAMEPAD_AXIS_LEFTY,
                                       +thumb_dead_zone, +32767);
    ImGui_ImplSDL3_UpdateGamepadAnalog(bd, io, ImGuiKey_GamepadRStickLeft, SDL_GAMEPAD_AXIS_RIGHTX,
                                       -thumb_dead_zone, -32768);
    ImGui_ImplSDL3_UpdateGamepadAnalog(bd, io, ImGuiKey_GamepadRStickRight, SDL_GAMEPAD_AXIS_RIGHTX,
                                       +thumb_dead_zone, +32767);
    ImGui_ImplSDL3_UpdateGamepadAnalog(bd, io, ImGuiKey_GamepadRStickUp, SDL_GAMEPAD_AXIS_RIGHTY,
                                       -thumb_dead_zone, -32768);
    ImGui_ImplSDL3_UpdateGamepadAnalog(bd, io, ImGuiKey_GamepadRStickDown, SDL_GAMEPAD_AXIS_RIGHTY,
                                       +thumb_dead_zone, +32767);
}
...
static void ImGui_ImplSDL3_UpdateMonitors() {
    ...
    for (int n = 0; n < display_count; n++) {
        // Warning: the validity of monitor DPI information on Windows dependson the application DPI
        ...
        // FIXME-VIEWPORT: On MacOS SDL reports actual monitor DPI scale, ignoring OS
        // configuration. We may want to set
        //  DpiScale to cocoa_window.backingScaleFactor here.
        monitor.DpiScale = SDL_GetDisplayContentScale(display_id);
        monitor.PlatformHandle = (void*)(intptr_t)n;
        if (monitor.DpiScale <= 0.0f)
            continue; // Some accessibility applications are declaring virtual monitors with a DPI
                      // of 0, see #7902.
...
void ImGui_ImplSDL3_NewFrame() {
    ImGui_ImplSDL3_Data* bd = ImGui_ImplSDL3_GetBackendData();
    IM_ASSERT(bd != nullptr &&
              "Context or backend not initialized! Did you callImGui_ImplSDL3_Init()?");
    ImGuiIO& io = ImGui::GetIO();
    ...
    // Update monitors
#ifdef WIN32
    bd->WantUpdateMonitors = true; // Keep polling under Windows to handle changes of work area
                                   // whenresizing task-bar (#8415)
#endif
    ...
    if (bd->MousePendingLeaveFrame && bd->MousePendingLeaveFrame >= ImGui::GetFrameCount() &&
    ...
    // Technically speaking testing for 'bd->MouseButtonsDown == 0' would be more rigorous, but
    // testingfor payload reduces noise and potential side-effects.
...
// This is an _advanced_ and _optional_ feature, allowing the backend to create andhandle multiple
...
// Helper structure we store in the void* PlatformUserData field of each ImGuiViewport to easily
// retrieveour backend data.
struct ImGui_ImplSDL3_ViewportData {
    SDL_Window* Window;
    SDL_Window* ParentWindow;
    Uint32 WindowID; // Stored in ImGuiViewport::PlatformHandle. Use SDL_GetWindowFromID()to get
                     // SDL_Window* from Uint32 WindowID.
    bool WindowOwned;
    SDL_GLContext GLContext;

    ImGui_ImplSDL3_ViewportData() {
...
static void ImGui_ImplSDL3_CreateWindow(ImGuiViewport* viewport) {
    ...
    ImGuiViewport* main_viewport = ImGui::GetMainViewport();
    ImGui_ImplSDL3_ViewportData* main_viewport_data =
        (ImGui_ImplSDL3_ViewportData*)main_viewport->PlatformUserData;
    ...
    vd->Window =
        SDL_CreateWindow("No Title Yet", (int)viewport->Size.x, (int)viewport->Size.y, sdl_flags);
...
static void ImGui_ImplSDL3_DestroyWindow(ImGuiViewport* viewport) {
    if (ImGui_ImplSDL3_ViewportData* vd =
...
static void ImGui_ImplSDL3_ShowWindow(ImGuiViewport* viewport) {
    ...
    // Note: SDL_WINDOW_UTILITY can be used to control task bar visibility, but on Windows,it does
    // not affect child windows.
...
// surface) SDL is graceful enough to _not_ need <vulkan/vulkan.h> so we can safely include this.
#include <SDL3/SDL_vulkan.h>
static int ImGui_ImplSDL3_CreateVkSurface(ImGuiViewport* viewport, ImU64 vk_instance,
                                          const void* vk_allocator, ImU64* out_vk_surface) {
    ImGui_ImplSDL3_ViewportData* vd = (ImGui_ImplSDL3_ViewportData*)viewport->PlatformUserData;
    (void)vk_allocator;
    bool ret = SDL_Vulkan_CreateSurface(vd->Window, (VkInstance)vk_instance,
