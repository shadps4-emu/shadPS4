...
std::shared_ptr<ShadPs4App> ShadPs4App::g_instance = std::make_shared<ShadPs4App>();

std::optional<int> ShadPs4App::parse(int argc, char* argv[]) {
...
int ShadPs4App::run() {
    ...
    if (bigPicture) {
        ...
        m_emulator.executableName = executableName;
        m_emulator.waitForDebuggerBeforeRun = waitForDebugger;
        m_emulator.Run(ebootPath, gameArgs, overrideRoot);
    }
