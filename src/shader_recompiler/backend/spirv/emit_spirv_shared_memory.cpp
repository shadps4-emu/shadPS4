...
Id EmitLoadSharedU16(EmitContext& ctx, Id offset) {
    ...
    const Id pointer = ctx.OpAccessChain(ctx.shared_u16, ctx.shared_memory_u32, index);
...
void EmitWriteSharedU16(EmitContext& ctx, Id offset, Id value) {
    ...
    const Id word_offset{ctx.OpShiftRightArithmetic(ctx.U16, offset, shift)};
    const Id pointer = ctx.OpAccessChain(ctx.shared_u16, ctx.shared_memory_u32, word_offset);
