...
static std::optional<s32> ReadConstant(const Gcn::InstOperand& operand,
    ...
    case Gcn::OperandField::Scc:
        ASSERT(operand.code < sources.size());
        return sources[operand.code];
    default:
        break;
    }
    UNREACHABLE_MSG("Unsupported copy shader constant operand field {}",
                    static_cast<u32>(operand.field));
}
    ...
    while (!code_slice.atEnd()) {
        ...
        default: {
            ...
            if (inst.src[3].field != Gcn::OperandField::ConstZero) {
                ...
                ASSERT_MSG(soffset.has_value(),
                           "Copy shader buffer load soffset could not be resolved");
                if (soffset) {
                    offset += *soffset;
                }
            }
