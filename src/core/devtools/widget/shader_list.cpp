...
ShaderList::Selection::Selection(int index)
    ...
    ShadPs4App::GetInstance()->m_hle_layer->m_gnm_driver.presenter->GetWindow().RequestKeyboard();
}

ShaderList::Selection::~Selection() {
    if (index >= 0) {
        ShadPs4App::GetInstance()->m_hle_layer->m_gnm_driver.presenter->GetWindow().ReleaseKeyboard();
    }
}
...
void ShaderList::Selection::ReloadShader(DebugStateType::ShaderDump& value) {
    ...
    auto& cache = ShadPs4App::GetInstance()->m_hle_layer->m_gnm_driver.presenter->GetRasterizer().GetPipelineCache();
