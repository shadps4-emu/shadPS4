...
private:
    ...
    public:
        ...
        s64 ReadV(s32 fd, const Libraries::Kernel::OrbisKernelIovec* iov, u32 iovcnt) override;
        s64 PReadV(s32 fd, const Libraries::Kernel::OrbisKernelIovec* iov, u32 iovcnt,
                   s64 offset) override;

        s64 Write(s32 fd, const void* buf, u64 count) override;
        s64 PWrite(s32 fd, const void* buf, u64 count, s64 offset) override;
        s64 WriteV(s32 fd, const Libraries::Kernel::OrbisKernelIovec* iov, u32 iovcnt) override;
        s64 PWriteV(s32 fd, const Libraries::Kernel::OrbisKernelIovec* iov, u32 iovcnt,
                    s64 offset) override;
        ...
        s32 FStat(s32 fd, Libraries::Kernel::OrbisKernelStat* statbuf) override;

        s32 Chmod(const fs::path& path, u16 mode) override;
        s32 FChmod(s32 fd, u16 mode) override;

        s64 GetDents(s32 fd, void* buf, u64 count, s64* basep) override;
...
public:
    ...
    int SyncHost(void);
    int SyncHost(const fs::path& path);
    ...
    int Mount(const fs::path& path, const partition_ptr& fs,
    ...
    int ForceInsert(const fs::path& path, const std::string& name, const inode_ptr& node);
    ...
    bool IsOpen(s32 fd) noexcept;
    int SetSize(s32 fd, s64 size) noexcept;
    s64 GetSize(s32 fd) noexcept;
    ...
    s64 GetSize(const fs::path& path) = delete;
    s64 GetSize(const fs::path& path, std::error_code& ec) noexcept = delete;
    int Exists(const fs::path& path) = delete;
    int Exists(const fs::path& path, std::error_code& ec) noexcept = delete;
    int IsDirectory(const fs::path& path) = delete;
    int IsDirectory(const fs::path& path, std::error_code& ec) noexcept = delete;
    fs::path AbsolutePath(const fs::path& path) = delete;
    fs::path AbsolutePath(const fs::path& path, std::error_code& ec) noexcept = delete;
    ...
    int CreateDirectory(const fs::path& path, int mode = 0777) = delete;
    int CreateDirectory(const fs::path& path, std::error_code& ec,
                        int mode = 0777) noexcept = delete;
    int CreateDirectory(const fs::path& path, const fs::path& existing_path,
                        int mode = 0777) = delete;
    int CreateDirectory(const fs::path& path, const fs::path& existing_path, std::error_code& ec,
                        int mode = 0777) noexcept = delete;
    int CreateDirectories(const fs::path& path, int mode = 0777) = delete;
    int CreateDirectories(const fs::path& path, std::error_code& ec,
                          int mode = 0777) noexcept = delete;
    fd_handle_ptr GetHandle(s32 fd);

private:
    void SyncHostImpl(const partition_ptr& part);
