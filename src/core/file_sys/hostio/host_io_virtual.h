...
public:
    explicit HostIO_Virtual(Resolved* resolved = nullptr, fd_handle_ptr handle = nullptr);
    ~HostIO_Virtual() override = default;
    //
    // Native wrappers
    //
     ...
     */
    s32 Open(const fs::path& path, s32 flags, u16 mode) override;
