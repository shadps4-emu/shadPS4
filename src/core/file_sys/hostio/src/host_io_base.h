...
public:
    ...
    virtual s32 Open(const fs::path& path, int flags, u16 mode);
    virtual s32 Creat(const fs::path& path, u16 mode);
    ...
    virtual s64 ReadV(s32 fd, const OrbisKernelIovec* iov, s32 iovcnt);
    virtual s64 PReadV(s32 fd, const OrbisKernelIovec* iov, s32 iovcnt, s64 offset);

    virtual s64 Write(s32 fd, const void* buf, u64 count);
    virtual s64 PWrite(s32 fd, const void* buf, u64 count, s64 offset);
    virtual s64 WriteV(s32 fd, const OrbisKernelIovec* iov, s32 iovcnt);
    virtual s64 PWriteV(s32 fd, const OrbisKernelIovec* iov, s32 iovcnt, s64 offset);

    virtual s32 MKDir(const fs::path& path, u16 mode);
