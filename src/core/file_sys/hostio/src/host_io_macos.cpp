...
s32 HostIO_MACOS::Open(const fs::path& path, s32 flags, u16 mode) {
    errno = 0;
    s32 status = open(path.c_str(), ToPOSIXOpenFlags(flags), mode);
    return status >= 0 ? status : -unix2bsd(errno);
}

s32 HostIO_MACOS::Creat(const fs::path& path, u16 mode) {
    errno = 0;
    s32 status = creat(path.c_str(), mode);
    return status >= 0 ? status : -unix2bsd(errno);
}

s32 HostIO_MACOS::Close(const s32 fd) {
    errno = 0;
    s32 status = close(fd);
    return 0 == status ? status : -unix2bsd(errno);
}

s32 HostIO_MACOS::Link(const fs::path& src, const fs::path& dst) {
    errno = 0;
    s32 status = link(src.c_str(), dst.c_str());
    return 0 == status ? status : -unix2bsd(errno);
}

s32 HostIO_MACOS::LinkSymbolic(const fs::path& src, const fs::path& dst) {
    errno = 0;
    s32 status = symlink(src.c_str(), dst.c_str());
    return 0 == status ? status : -unix2bsd(errno);
}

s32 HostIO_MACOS::Unlink(const fs::path& path) {
    errno = 0;
    s32 status = unlink(path.c_str());
    return 0 == status ? status : -unix2bsd(errno);
}

s32 HostIO_MACOS::Remove(const fs::path& path) {
    errno = 0;
    s32 status = remove(path.c_str());
    return 0 == status ? status : -unix2bsd(errno);
}

s32 HostIO_MACOS::Flush(const s32 fd) {
    errno = 0;
    return 0;
}

s32 HostIO_MACOS::FSync(const s32 fd) {
    errno = 0;
    s32 status = fsync(fd);
    return 0 == status ? status : -unix2bsd(errno);
}

s64 HostIO_MACOS::LSeek(const s32 fd, s64 offset, s32 whence) {
    errno = 0;
    s32 status = lseek(fd, offset, ToPOSIXSeekOrigin(whence));
    return status >= 0 ? status : -unix2bsd(errno);
}

s64 HostIO_MACOS::Tell(const s32 fd) {
    return LSeek(fd, 0, SeekOrigin::CURRENT);
}

s32 HostIO_MACOS::Truncate(const fs::path& path, u64 size) {
    errno = 0;
    s32 status = truncate(path.c_str(), size);
    return status >= 0 ? status : -unix2bsd(errno);
}

s32 HostIO_MACOS::FTruncate(const s32 fd, u64 size) {
    errno = 0;
    s32 status = ftruncate(fd, size);
    return status >= 0 ? status : -unix2bsd(errno);
}

s64 HostIO_MACOS::Read(const s32 fd, void* buf, u64 count) {
    errno = 0;
    s32 status = read(fd, buf, count);
    return status >= 0 ? status : -unix2bsd(errno);
}

s64 HostIO_MACOS::PRead(const s32 fd, void* buf, u64 count, s64 offset) {
    errno = 0;
...
s64 HostIO_MACOS::Write(const s32 fd, const void* buf, u64 count) {
    errno = 0;
    s32 status = write(fd, buf, count);
    return status >= 0 ? status : -unix2bsd(errno);
}

s64 HostIO_MACOS::PWrite(const s32 fd, const void* buf, u64 count, s64 offset) {
    errno = 0;
    s32 status = pwrite(fd, buf, count, offset);
    return status >= 0 ? status : -unix2bsd(errno);
}

s64 HostIO_MACOS::WriteV(const s32 fd, const OrbisKernelIovec* iov, u32 iovcnt) {
    errno = 0;
...
s64 HostIO_MACOS::PWriteV(const s32 fd, const OrbisKernelIovec* iov, u32 iovcnt, s64 offset) {
    errno = 0;
...
s32 HostIO_MACOS::MKDir(const fs::path& path, u16 mode) {
    errno = 0;
    s32 status = mkdir(path.c_str(), mode);
    return 0 == status ? status : -unix2bsd(errno);
}

s32 HostIO_MACOS::RMDir(const fs::path& path) {
    errno = 0;
    s32 status = rmdir(path.c_str());
    return 0 == status ? status : -unix2bsd(errno);
}

s32 HostIO_MACOS::Stat(const fs::path& path, OrbisKernelStat* statbuf) {
    errno = 0;
...
s32 HostIO_MACOS::FStat(const s32 fd, OrbisKernelStat* statbuf) {
    errno = 0;
...
s32 HostIO_MACOS::Chmod(const fs::path& path, u16 mode) {
    errno = 0;
    s32 status = chmod(path.c_str(), mode);
    return 0 == status ? status : -unix2bsd(errno);
}

s32 HostIO_MACOS::FChmod(const s32 fd, u16 mode) {
    errno = 0;
    s32 status = fchmod(fd, mode);
    return 0 == status ? status : -unix2bsd(errno);
}

s32 HostIO_MACOS::Copy(const fs::path& src, const fs::path& dst, bool fail_if_exists) {
    return -unix2bsd(ENOSYS);
}

s32 HostIO_MACOS::Move(const fs::path& src, const fs::path& dst, bool fail_if_exists) {
    return -unix2bsd(ENOSYS);
}

// s32 HostIO_MACOS::GetDents(void* buf, u32 count, s64* basep) { return -POSIX_ENOSYS; }
