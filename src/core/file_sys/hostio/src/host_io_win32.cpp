...
s32 HostIO_Win32::Flush(s32 fd) {
    errno = 0;
    return 0;
}

s32 HostIO_Win32::FSync(s32 fd) {
    errno = 0;
    s32 status = _commit(fd);
    return 0 == status ? 0 : -unix2bsd(errno);
}

s64 HostIO_Win32::LSeek(s32 fd, s64 offset, s32 whence) {
    errno = 0;
    s32 status = _lseeki64(fd, offset, ToWIN32SeekOrigin(whence));
    return status >= 0 ? status : -unix2bsd(errno);
}

s64 HostIO_Win32::Tell(s32 fd) {
...
int HostIO_Win32::FTruncate(s32 fd, s64 size) {
    errno = 0;
    s32 status = _chsize_s(fd, size);
    return status >= 0 ? status : -unix2bsd(errno);
}

s64 HostIO_Win32::Read(s32 fd, void* buf, u64 count) {
    errno = 0;
    s32 status = _read(fd, buf, count);
    return status >= 0 ? status : -unix2bsd(errno);
}

s64 HostIO_Win32::PRead(s32 fd, void* buf, u64 count, s64 offset) {
    errno = 0;
...
s64 HostIO_Win32::ReadV(s32 fd, const OrbisKernelIovec* iov, u32 iovcnt) {
    return 0;
}
s64 HostIO_Win32::PReadV(s32 fd, const OrbisKernelIovec* iov, u32 iovcnt, s64 offset) {
    return 0;
}

s64 HostIO_Win32::Write(s32 fd, const void* buf, u64 count) {
    errno = 0;
    s32 status = _write(fd, buf, count);
    return status >= 0 ? status : -unix2bsd(errno);
}

s64 HostIO_Win32::PWrite(s32 fd, const void* buf, u64 count, s64 offset) {
    errno = 0;
    s64 bak = LSeek(fd, 0, SeekOrigin::CURRENT);
    if (bak < 0)
        return -unix2bsd(errno);
...
s64 HostIO_Win32::WriteV(s32 fd, const OrbisKernelIovec* iov, u32 iovcnt) {
    return 0;
}
s64 HostIO_Win32::PWriteV(s32 fd, const OrbisKernelIovec* iov, u32 iovcnt, s64 offset) {
    return 0;
}
...
// s32 HostIO_Win32::FStat(s32 fd, Libraries::Kernel::OrbisKernelStat* statbuf) {}

// s32 HostIO_Win32::Chmod(const fs::path& path, u16 mode) {}
// s32 HostIO_Win32::FChmod(s32 fd, u16 mode) {}
