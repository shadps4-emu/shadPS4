...
struct WaveformBlockInternal {
    ...
    WaveformBlockInternal() : dataOffset(0), dataSize(0), numRepeats(0),
                               numSkipSamples(0), numSamples(0), currentRepeat(0) {}
};
...
struct RingBufferSlot {
    ...
    RingBufferSlot() : basePtr(nullptr), data(nullptr), dataSize(0), numSamples(0), 
                       valid(false), consumed(false) {}
...
struct VoiceInternal {
    ...
    VoiceInternal() : ownerRack(nullptr), voiceIndex(0), stateFlags(0), 
                      pitchRatio(1.0f), portVolume(1.0f), isSetup(false),
                      currentSamplePos(0), samplePosFloat(0.0f),
                      currentBlockIndex(0), flags(0), isStreaming(false),
                      ringWriteIndex(0), ringReadIndex(0), ringBufferCount(0),
    ...
    void advanceReadIndex() {
        if (!ringBuffer[ringReadIndex].valid) return;
