...
PthreadAttr PthreadAttrDefault = {
    ...
    .sched_inherit = SHAD_PTHREAD_INHERIT_SCHED,
    .prio = 0,
    .suspend = false,
...
int PS4_SYSV_ABI posix_pthread_attr_setschedparam(PthreadAttrT* attr, const PthreadSchedParam* param) {
    if (attr == nullptr || *attr == nullptr) {
        return POSIX_EINVAL;
    }
    if (param == nullptr) {
...
int PS4_SYSV_ABI scePthreadAttrGetaffinity(PthreadAttrT* pattr, Cpuset* mask) {
    Cpuset cpuset;
    const int ret = posix_pthread_attr_getaffinity_np(pattr, 0x10, &cpuset);
    if (ret == 0) {
        *mask = cpuset;
    }
    return ret;
