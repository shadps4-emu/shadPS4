...
s32 PS4_SYSV_ABI sceAudio3dAudioOutClose(const s32 handle) {
    ...
    if (ShadPs4App::GetInstance()->m_emulator.m_hle_layer->m_audio3d_openal->state) {
        for (auto& [port_id, port] : ShadPs4App::GetInstance()->m_emulator.m_hle_layer->m_audio3d_openal->state->ports) {
...
s32 PS4_SYSV_ABI sceAudio3dAudioOutOpen(
    ...
    if (!ShadPs4App::GetInstance()->m_emulator.m_hle_layer->m_audio3d_openal->state->ports.contains(port_id)) {
        LOG_ERROR(Lib_Audio3d, "!state->ports.contains(port_id)");
        return ORBIS_AUDIO3D_ERROR_INVALID_PORT;
    }

    std::scoped_lock lock{ShadPs4App::GetInstance()->m_emulator.m_hle_layer->m_audio3d_openal->state->ports[port_id].mutex};
    if (len != ShadPs4App::GetInstance()->m_emulator.m_hle_layer->m_audio3d_openal->state->ports[port_id].parameters.granularity) {
    ...
    ShadPs4App::GetInstance()->m_emulator.m_hle_layer->m_audio3d_openal->state->ports[port_id].audioout_handles.push_back(handle);
    return handle;
}
...
s32 PS4_SYSV_ABI sceAudio3dBedWrite2(const OrbisAudio3dPortId port_id, const u32 num_channels,
    ...
    std::scoped_lock lock{ShadPs4App::GetInstance()->m_emulator.m_hle_layer->m_audio3d_openal->state->ports[port_id].mutex};
    return ConvertAndEnqueue(ShadPs4App::GetInstance()->m_emulator.m_hle_layer->m_audio3d_openal->state->ports[port_id].bed_queue,
                             OrbisAudio3dPcm{
                                 .format = format,
                                 .sample_buffer = buffer,
                                 .num_samples = num_samples,
                             },
                             num_channels, ShadPs4App::GetInstance()->m_emulator.m_hle_layer->m_audio3d_openal->state->ports[port_id].parameters.granularity);
}
...
s32 PS4_SYSV_ABI sceAudio3dInitialize(const s64 reserved) {
    ...
    ShadPs4App::GetInstance()->m_emulator.m_hle_layer->m_audio3d_openal->state = std::make_unique<Audio3dState>();
...
s32 PS4_SYSV_ABI sceAudio3dObjectReserve(const OrbisAudio3dPortId port_id,
    ...
    *object_id = ORBIS_AUDIO3D_OBJECT_INVALID;
    ...
    auto& port = ShadPs4App::GetInstance()->m_emulator.m_hle_layer->m_audio3d_openal->state->ports[port_id];
    std::scoped_lock lock{port.mutex};
...
s32 PS4_SYSV_ABI sceAudio3dPortClose(const OrbisAudio3dPortId port_id) {
    ...
    ShadPs4App::GetInstance()->m_emulator.m_hle_layer->m_audio3d_openal->state->ports.erase(port_id);
    return ORBIS_OK;
}
...
s32 PS4_SYSV_ABI sceAudio3dPortGetQueueLevel(const OrbisAudio3dPortId port_id, u32* queue_level,
    ...
    const auto& port = ShadPs4App::GetInstance()->m_emulator.m_hle_layer->m_audio3d_openal->state->ports[port_id];
    std::scoped_lock lock{port.mutex};
...
s32 PS4_SYSV_ABI sceAudio3dPortOpen(const Libraries::UserService::OrbisUserServiceUserId user_id,
    ...
    const int id = static_cast<int>(ShadPs4App::GetInstance()->m_emulator.m_hle_layer->m_audio3d_openal->state->ports.size()) + 1;
    ...
    *port_id = id;
    auto& port = ShadPs4App::GetInstance()->m_emulator.m_hle_layer->m_audio3d_openal->state->ports[id];
    std::memcpy(
...
s32 PS4_SYSV_ABI sceAudio3dTerminate() {
    ...
    for (const auto& [id, _] : ShadPs4App::GetInstance()->m_emulator.m_hle_layer->m_audio3d_openal->state->ports) {
    ...
    ShadPs4App::GetInstance()->m_emulator.m_hle_layer->m_audio3d_openal->state.reset();
    return ORBIS_OK;
}
